...
export class TechRadarValidationService {
  ...
  validate(entity: Partial<TechRadarEntity>, isUpdate: boolean = false): ValidationResult {
    ...
    if (entity.type !== undefined && !VALID_TYPES.includes(entity.type as any)) {
      errors.push({ field: 'type', message: `Недопустимое значение type: ${entity.type}` });
    }
    if (entity.subtype !== undefined && !VALID_SUBTYPES.includes(entity.subtype as any)) {
      errors.push({ field: 'subtype', message: `Недопустимое значение subtype: ${entity.subtype}` });
    }
    if (entity.category !== undefined && !VALID_CATEGORIES.includes(entity.category as any)) {
      errors.push({ field: 'category', message: `Недопустимое значение category: ${entity.category}` });
    }
    if (entity.maturity !== undefined && !VALID_MATURITY.includes(entity.maturity as any)) {
      errors.push({ field: 'maturity', message: `Недопустимое значение maturity: ${entity.maturity}` });
    }
    if (entity.riskLevel !== undefined && !VALID_RISK_LEVEL.includes(entity.riskLevel as any)) {
      errors.push({ field: 'riskLevel', message: `Недопустимое значение riskLevel: ${entity.riskLevel}` });
    }
    if (entity.supportStatus !== undefined && !VALID_SUPPORT_STATUS.includes(entity.supportStatus as any)) {
      errors.push({ field: 'supportStatus', message: `Недопустимое значение supportStatus: ${entity.supportStatus}` });
    }
    if (entity.performanceImpact !== undefined && !VALID_PERFORMANCE_IMPACT.includes(entity.performanceImpact as any)) {
      errors.push({ field: 'performanceImpact', message: `Недопустимое значение performanceImpact: ${entity.performanceImpact}` });
    }
    if (entity.contributionFrequency !== undefined && !VALID_CONTRIBUTION_FREQUENCY.includes(entity.contributionFrequency as any)) {
      errors.push({ field: 'contributionFrequency', message: `Недопустимое значение contributionFrequency: ${entity.contributionFrequency}` });
    }
    if (entity.costFactor !== undefined && !VALID_COST_FACTOR.includes(entity.costFactor as any)) {
      errors.push({ field: 'costFactor', message: `Недопустимое значение costFactor: ${entity.costFactor}` });
    }
    if (entity.businessCriticality !== undefined && !VALID_BUSINESS_CRITICALITY.includes(entity.businessCriticality as any)) {
