...
export class TechRadarValidationService {
   ...
   */
  validate(entity: Partial<TechRadarEntity>, isUpdate: boolean = false): ValidationResult {
    ...
    if (entity.type !== undefined && entity.type !== null && !TECH_RADAR_TYPES.includes(entity.type as any)) {
      errors.push({ field: 'type', message: `Недопустимое значение type: ${entity.type}` });
    }
    if (entity.subtype !== undefined && entity.subtype !== null && !TECH_RADAR_SUBTYPES.includes(entity.subtype as any)) {
      errors.push({ field: 'subtype', message: `Недопустимое значение subtype: ${entity.subtype}` });
    }
    if (entity.category !== undefined && entity.category !== null && !TECH_RADAR_CATEGORIES.includes(entity.category as any)) {
      errors.push({ field: 'category', message: `Недопустимое значение category: ${entity.category}` });
    }
    if (entity.maturity !== undefined && entity.maturity !== null && !TECH_RADAR_MATURITY.includes(entity.maturity as any)) {
      errors.push({ field: 'maturity', message: `Недопустимое значение maturity: ${entity.maturity}` });
    }
    if (entity.riskLevel !== undefined && entity.riskLevel !== null && !TECH_RADAR_RISK_LEVEL.includes(entity.riskLevel as any)) {
      errors.push({ field: 'riskLevel', message: `Недопустимое значение riskLevel: ${entity.riskLevel}` });
    }
    if (entity.supportStatus !== undefined && entity.supportStatus !== null && !TECH_RADAR_SUPPORT_STATUS.includes(entity.supportStatus as any)) {
      errors.push({ field: 'supportStatus', message: `Недопустимое значение supportStatus: ${entity.supportStatus}` });
    }
    if (entity.performanceImpact !== undefined && entity.performanceImpact !== null && !TECH_RADAR_PERFORMANCE_IMPACT.includes(entity.performanceImpact as any)) {
      errors.push({ field: 'performanceImpact', message: `Недопустимое значение performanceImpact: ${entity.performanceImpact}` });
    }
    if (entity.contributionFrequency !== undefined && entity.contributionFrequency !== null && !TECH_RADAR_CONTRIBUTION_FREQUENCY.includes(entity.contributionFrequency as any)) {
      errors.push({ field: 'contributionFrequency', message: `Недопустимое значение contributionFrequency: ${entity.contributionFrequency}` });
    }
    if (entity.costFactor !== undefined && entity.costFactor !== null && !TECH_RADAR_COST_FACTOR.includes(entity.costFactor as any)) {
      errors.push({ field: 'costFactor', message: `Недопустимое значение costFactor: ${entity.costFactor}` });
    }
    if (entity.businessCriticality !== undefined && entity.businessCriticality !== null && !TECH_RADAR_BUSINESS_CRITICALITY.includes(entity.businessCriticality as any)) {
